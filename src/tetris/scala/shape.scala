...
object ShapeLib {
  ...
  def combine(shape1:Shape,shape2:Shape):Shape={
   ...
   // 6. eraseRows
  // 目的：揃った行を消去する
  def eraseRows(pile: Shape): Shape = {
    def rowcheck(row:Row):Boolean ={
      row.contains(Transparent) || row.length != 10
    }
    empty(10-pile.count(rowcheck),10)++pile.filter(rowcheck)
  }
}
// テスト
object ShapeTest extends App {
  ...
  */
  println("eraseRows")
  println(eraseRows(List(duplicate(9,Blue)++List(Blue))))
}
